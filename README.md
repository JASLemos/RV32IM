...
A simple, scalar, in order, 5 stage pipelined RISC-V RV32IM CPU designed for academic purposes, synthesized in **Vivado Design Suite** for the **Nexys7-50T**. The CPU is described in **Verilog HDL** and includes the following key components:

- **Instruction Memory**: Synthesized as a **ROM** using Distributed Memory via Vivado IP Catalog.
- **Data Memory**: Synthesized as a **byte addressable RAM** described in Verilog, where the byte enable signals follow **One Hot encoding** .
...
- **Multiplier and Divider**: The Multiplier circuit is a high speed **Modified Booth Encoding Wallace Tree Multiplier**, and the Divider is a slow sequential circuit
that implements **Restoring Division Algorithm**.
...
- **Components**:
    ...
    - **Multiplier:** Modified Booth Encoded Wallace Tree Multiplier which is segmentated into 2 *pipeline* stages, to achieve higher *Clock* frequencies.
 
    - **Divider:** Sequential Divider circuit which implements the restoring division algorithm, where every **div{U}/rem{u}** takes 32 *Clock* cycles.
